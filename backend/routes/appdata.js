...
const ALLOWED_APPS = ['notes', 'journal', 'todo', 'photos', 'doodle', 'calculator', 'trivia', 'memory', 'snake', 'tictactoe'];
const MAX_DATA_SIZE = 500000; // ~500KB limit per app

// GET /api/appdata?uuid=X&app=Y — Load data for one app
router.get('/', async (req, res) => {
    try {
        const { uuid, app } = req.query;
        if (!uuid || !app) return res.status(400).json({ error: 'Missing uuid or app' });
